...
<html lang="es">
    ...
    <body>
        ...
        <script type="module">
            ...
            // Nueva función para guardar valores de checkboxes en el arreglo de 240 bools
            async function saveCheckboxValues() {
                const boolArray = new Array(240).fill(false); // Inicializar arreglo de 240 elementos
                const checkboxes = document.querySelectorAll('input[type="checkbox"]:not(.available)'); // Filtrar checkboxes no deshabilitados
                
                checkboxes.forEach(checkbox => {
                    const index = parseInt(checkbox.className, 10);
                    if (!isNaN(index) && index >= 0 && index < 240) {
                        boolArray[index] = checkbox.checked; // Asignar el valor correspondiente
                    }
                });
                
                console.log(boolArray); // Mostrar el arreglo para depuración
